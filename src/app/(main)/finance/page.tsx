  ...
  return (
    <div className="p-6 text-gray-900 dark:text-white">
      ...
      <section className="mb-8">
        ...
        <div className="bg-white dark:bg-gray-800/50 border rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-100 dark:bg-gray-700/50">
                <tr>
                  <th className="p-4 text-left">Product</th>
                  <th className="p-4 text-left">Sold Qty</th>
              ...
              <tbody>
                {profits.map((p) => (
                  <tr key={p.ProductId} className="border-t hover:bg-gray-50 dark:hover:bg-gray-700/30">
      ...
      <section className="mb-12">
        ...
        {/* Form */}
        <div className="bg-white dark:bg-gray-800/50 border rounded-xl p-5 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <input
              type="number"
              placeholder="Enter amount"
              value={handover.amount}
              onChange={(e) => setHandover({ ...handover, amount: e.target.value })}
            ...
            <input
              ...
              onChange={(e) => setHandover({ ...handover, manager: e.target.value })}
              className="w-full bg-gray-50 dark:bg-gray-800/50 border border-gray-300 dark:border-gray-700 rounded-lg px-4 py-3 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all"
            />
            <button onClick={saveHandover} className="btn-primary">
              Record Handover
            </button>
          </div>
        </div>
          ...
          {handovers.map((h) => (
            <div key={h.Id} className="card">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-lg font-bold">Rs {h.Amount}</span>
              </div>
              <p className="text-sm text-gray-700">Manager ID: {h.UserId}</p>
        ...
        <div className="bg-white dark:bg-gray-800/50 border rounded-xl p-5 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <input
              ...
              className="w-full bg-gray-50 dark:bg-gray-800/50 border border-gray-300 dark:border-gray-700 rounded-lg px-4 py-3 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all"
            />
            <button onClick={saveCash} className="btn-primary">
              Record Cash
            </button>
          </div>
        </div>
          ...
          {cashUsages.map((c) => (
            <div key={c.Id} className="card">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-lg font-bold">Rs {c.Amount}</span>
              </div>
...
function Card({ title, value, icon, iconBg, iconColor }: any) {
  return (
    <div className="bg-white dark:bg-gray-800/50 border rounded-xl shadow-sm p-5">
